...
softcore.param <- list(cif="sftcr", par=list(beta=Nbre_points/area(window), kappa=0.2, sigma=0.01), w=window)
softcore.mod <- rmh(softcore.param, start=list(n.start=Nbre_points), control=list(nrep=1e6), verbose=FALSE)
Sk_ux.softcore <- Create.basis(softcore.mod, Y, Rmax = 0.10, rmin = 0.0001, 50, xrange = c(0,2), yrange = c(0,2))
theta.est.softcore <- ppm(softcore.mod ~ ., covariates = Sk_ux.softcore)
theta.true.softcore <- ppm(softcore.mod ~ 1, Softcore(kappa=0.2,sigma=0.01), rbord=0.15)
par(mfrow = c(1,2))
...
Sk_ux.dgs <- Create.basis(dgs.mod, Y, Rmax = 0.10, rmin = 0.0001, 50, xrange = c(0,2), yrange = c(0,2))
...
curve(Soft.int(x,sigma=.05,kappa=0.2), 0, 0.15, main="softcore interaction",ylim=c(-1,1))
curve(Phi.hat(x,R=0.10,rmin=0.0001), from=0, to=0.15, lty="dashed", add=TRUE)
